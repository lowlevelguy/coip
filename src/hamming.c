...
uint8_t hamming_encode_nibble(uint8_t data) {
	...
	for (int i = 0; i < 7; i++) {
		total_parity ^= ((encoded >> i) & 1); // Inline getBit
	}
	encoded |= (total_parity << 7);
...
uint8_t hamming_correct(uint8_t encoded) {
	// Count of "on" bits in positions 1–7 (indices 0–6)
	int onCount = 0;
	int onList[8];
	...
	for (int i = 0; i < onCount; i++) {
     ...
     if (errorLoc) {
            ...
            if (!((onCount & 1) ^ (encoded & (2 << 7)))) { // last bit of onCount (total parity) XOR first bit of block (parity bit)
                printf("\nMore than one error detected. Continuing with corrupted byte.\n");
                return encoded;
            }
